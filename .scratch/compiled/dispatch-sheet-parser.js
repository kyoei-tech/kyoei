...
function splitNameArea(items) {
    const lines = clusterByCoordinate(items, (item) => item.y, LINE_CLUSTER_GAP);
    if (lines.length === 0)
        return { auctionInfo: '', vehicleName: '' };
    ...
    if (currentRun.length > 0)
        runs.push(currentRun);
    const auctionInfo = runs.length >= 2 ? joinColumnItems(runs.slice(0, -1).flat()) : '';
...
function parseVehicleRows(items, headerY, ranges) {
    ...
    for (const rowItems of rowClusters) {
        ...
        for (const item of rowItems) {
            ...
            else if (inRange(item.x, ranges.nameArea))
                byColumn.nameArea.push(item);
