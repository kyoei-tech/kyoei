...
const { parseExtractedPages } = await import('./compiled/dispatch-sheet-parser.js')
...
fs.writeFileSync('/vercel/share/v0-project/.scratch/sample1-result.json', JSON.stringify(result, null, 2))
...
console.log('unknown chassisNumber count:', result.rounds.flatMap(r => r.vehicles).filter(v => !v.chassisNumber).length)
